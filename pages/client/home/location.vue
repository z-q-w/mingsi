	...
	<view class="page">
		...
		<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight+'px'}">
			<view class="search-bar">
				<image @click="backtohome" style="width: 36rpx; height: 36rpx;margin-right: 20rpx;" src="../../../static/image/a8a2cabe09b2ad058b3d2cca4acc9fa.png" mode="aspectFit"></image>
...
<script>
	...
	export default {
			...
			setTimeout(() => {
				uni.getSystemInfo({
					success: function(res) {
						...
						that.titleHeight = uni.upx2px(132);  //未知
		...
		methods: {
			...
			selectCity(e) {
				this.api.hasDept(
					JSON.stringify(e.currentTarget.dataset.name)
				).then(res=>{
					if(res.data){
						const city = e.currentTarget.dataset.name
						uni.$emit('upaddress',{mag:city})
						uni.navigateBack({
							delta:1
						})
					}else{
						this.tui.toast('当前城市没有机构')
					}
				})
			...
			touchStart(e) {
				...
				let item = this.lists[index === 0 ? 1 : index-1]
				if (item) {
